...
Canonical repository: https://github.com/none0enon/math-reader-codex
...
- The repository is public, as requested by the owner. Its independent Pages URL is `https://none0enon.github.io/math-reader-codex/`; the `Deploy GitHub Pages` workflow tests and publishes only `docs/`, never the gateway or private state. Commit the APK web assets and their `docs/` mirror together: the deploy workflow fails closed on divergence. A bot-only mirror synchronization does not automatically trigger another Pages run; after fixing a mismatch, dispatch this workflow on `main`.
...
- `Sign APK` validates `com.mathreader.boox.codex` and signs using alias `math-reader-codex-release`. Its dedicated certificate SHA-256 is `D45BD67FEB2C0E4B4871F0EFC454F93B4BD3FCEF9197084882547FB5EC38DD7B`. The application ID and signing alias are retained for installed-data and update compatibility; they are not display names. The stable app's signing key was not read or reused. Keep an offline private backup of the new key/password to preserve future updates.

## Before distributing the application

The owner selected side-by-side installation. The application ID is `com.mathreader.boox.codex`, the launcher label is `math-reader-codex`, and the app uses the original Math Reader icon. The Java namespace remains `com.mathreader.boox`; the manifest explicitly names its component classes. The merged AndroidX provider authority is `com.mathreader.boox.codex.androidx-startup`; no shared UID or legacy provider/deep-link authority collision was found. Android allocates independent app data by application ID. Local debug/release builds and packaged identity checks pass. Actual BOOX installation, launch, handwriting and uninstall/data-isolation acceptance remain required; never uninstall the stable app to resolve a signature/version conflict. Do not configure both apps to overwrite the same cloud-sync target.

The PWA uses relative start/scope URLs. Because browser storage is shared by origin rather than repository path, it retains the existing `math-reader-boox-codex:v1:` namespace for Web Storage, IndexedDB and cache names so current browser data remains compatible after the repository rename. It never automatically reads or migrates unprefixed stable data. The service worker only deletes caches with this prefix. Unit tests and a disposable real Chrome context verify that stable settings, database records, and caches survive this PWA's initialization, saving and reloads. This is application-data separation, not a security boundary against malicious scripts sharing the same origin; use a separate domain for that stronger boundary. Do not publish over the stable Pages site.
